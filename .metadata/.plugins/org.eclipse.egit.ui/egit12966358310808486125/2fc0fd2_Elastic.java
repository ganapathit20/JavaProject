...
public class Elastic{
	...
	public void save(IStorageObject entity) throws StorageException{
		...
		final String index = INDEXES.get(entity);

		if(null == index) {
			throw new StorageException(String.format("Index for entity: %s not configured"));
		}

		try {

			Request r = new Request("POST", String.format("%s/_update_by_query/%s", index));
